...
class CockpitAttachBridge:
    ...
    def __init__(
        ...
        *,
        ...
        # Fire-and-forget attach-briefing tasks, kept referenced so a
        # scheduled greeting is not garbage-collected before it runs.
        self._briefing_tasks: Set["asyncio.Task"] = set()
    ...
    def _active_model() -> str:
        """The model to NAME on the banner: the tag the boot gate resolved,
        else the generation lane's configured model (so an idle daemon, or one
        whose registry blipped at boot, still names what it will answer with).
        Daemon-side and config-driven — never the client's guess. NEVER raises.
        """
        try:
    ...
    async def _on_client(
        ...
        try:
            ...
            # PROACTIVE GREETING. The organism says -- in its own voice,
            # generated by the LOCAL model against a live situation digest --
            # what it is working on or about to do. Scheduled, never awaited:
            # a cold model makes the briefing arrive late, never the attach.
            # Emitted as a NarrativeChannel INTENT frame, so the transport's
            # existing commit listener carries it to this cockpit through the
            # same mirror the per-op intent uses. NEVER breaks the attach.
            try:
                from backend.core.ouroboros.governance.attach_briefing import (
                    schedule_for_attach,
                )
                _brief_task = schedule_for_attach(session="")
                if _brief_task is not None:
                    self._briefing_tasks.add(_brief_task)
                    _brief_task.add_done_callback(self._briefing_tasks.discard)
            except Exception:  # noqa: BLE001 -- a greeting never breaks an attach
                logger.debug("[CockpitAttach] attach briefing skipped",
                             exc_info=True)
